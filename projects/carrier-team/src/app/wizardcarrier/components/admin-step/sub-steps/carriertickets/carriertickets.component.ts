...
export class CarrierTicketComponent implements OnInit {
  ...
  fetchAllTickets() {
    ...
    this.wizardBackendService.getAllTickets().subscribe(
      (response: any) => {
        console.log('Fetched new ticket data from Wizard Backend');
  ...
  fetchFilteredUnassignedTickets() {
    ...
    this.wizardBackendService.getFilteredUnassignedTickets().subscribe(
      (response: any) => {
        ...
        this.zendeskTicketCount = this.tickets.length; // Uppdatera antal tickets
        this.isLoading = false; // Data laddad
      },
      error => {
        console.error('Error fetching filtered unassigned tickets:', error);
        this.isLoading = false; // Säkerställ att laddningsindikatorn stängs
      },
    );
  }
