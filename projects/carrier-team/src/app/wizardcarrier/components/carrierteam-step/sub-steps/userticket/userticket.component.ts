...
export class UserComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.activeFilter = 'needUpdate'; // Set default active filter
    ...
    const observer = new MutationObserver(() => {
      ...
      this.wizardBackendService.GetZendeskCarrierMangement().subscribe({
        next: response => {
  ...
  private filterMatchesActive(ticket: any): boolean {
    switch (this.activeFilter) {
      ...
      case 'new':
        return this.isNewTicket(ticket);
      case 'needUpdate':
      ...
      case 'closedJira':
        return ticket.jiraData?.length > 0 && ticket.jiraData.every(jira => jira.sprints.ticketStatus === 'Closed');
  ...
  private calculateETA(tickets: any[]): any[] {
    return tickets.map(ticket => {
      ...
      ticket.jiraData.forEach((jira: any) => {
        ...
        const futureSprintEndDates = jira.sprints?.futureSprints?.map((sprint: any) => sprint.endDate) || [];
      ...
      let updatedETA = latestEndDate ? new Date(latestEndDate) : null;
      if (updatedETA) {
        updatedETA.setDate(updatedETA.getDate() + 14);
      }

      return {
        ...ticket,
        eta: updatedETA ? updatedETA.toISOString() : null,
      };
    });
  }
  ...
  isNewTicket(ticket: any): boolean {
    if (!ticket.reporterHasResponded) {
      return true;
    }
  ...
  private countTicketsToBeAnswered() {
    ...
    if (!this.ticketsToBeAnsweredCount) {
      this.activeFilter = 'total';
    }
  }

  private countTicketsNew() {
    this.ticketsNewCount = this.assignedTicketsBackup.filter(ticket => this.isNewTicket(ticket)).length;
  }
  ...
  filterTickets(filter: string) {
    if (
      ...
      (filter === 'new' && (this.ticketsNewCount || 0) === 0) ||
    ...
    switch (filter) {
      ...
      case 'needUpdate':
        ...
        break;
      case 'new':
        this.assignedTickets = this.assignedTicketsBackup.filter(ticket => this.isNewTicket(ticket));
        break;
      case 'urgent':
